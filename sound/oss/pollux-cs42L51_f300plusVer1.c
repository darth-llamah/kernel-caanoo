...
static void pollux_set_volume(struct work_struct *work)
{
	...
	dev->ADC_reading = reading;
    	...
    	volume_reg_val = dev->mixer.volTable[index];
	now_vol = index *  dev->mixer.vol_div100;
	
	if( dev->mixer.volume != now_vol ){
            
	    if( now_vol == 0 )
		pollux_gpio_setpin(SND_SPK_ON_OFF, 0);
	    else if( (dev->mixer.volume == 0 ) && (dev->isOpen) )
		pollux_gpio_setpin(SND_SPK_ON_OFF, 1);
			    
	    dev->mixer.volume = now_vol;
...
static int cs42L51_init(void)
{
	...
	if(dev->mixer.step_nums == 10 )
		dev->mixer.volTable = VolumeTable10;
	else if(dev->mixer.step_nums == 20)
		dev->mixer.volTable = VolumeTable20;
