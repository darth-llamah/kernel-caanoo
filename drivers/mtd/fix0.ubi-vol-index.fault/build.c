 ...
 *
 * Author: Artem Bityutskiy (–ë–∏—Ç—é—Ü–∫–∏–π –ê—Ä—Ç—ë–º),
 *         Frank Haverkamp
 */
...
gprintk("2 vol->vol_index = %d, calc vol_index = ubi_num + vol_id = %d + %d = %d\n", vol->vol_index, vol->ubi->ubi_num, vol->vol_id, vol->vol_id + vol->ubi->ubi_num);

	//if( vol->ubi->ubi_num != 1 )
	//	return 0; // just return

	mutex_lock(&vol_table_mutex);
	vol->vol_index = vol->ubi->ubi_num + vol->vol_id; // ghcstop add
	if( !vol_table[vol->vol_index] )  // vol->vol_id∞° 0¿Œ ≥‡ºÆ¿Ã NULL¿Œ ∞ÊøÏ
	{
		struct list_head *this;

		vol_table[vol->vol_index] = vol;
