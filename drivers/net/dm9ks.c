...
/* from davicom dm9000a
<Linux Base Driver for DM9000A/DM9010 adapter>

DAVICOM Semiconductor Inc.				03/14/2005
...
<B. Building and Installation>
  ...
  B-5: Install the module:
       -Kernel 2.4
       insmod dm9ks.o <parameter>=<value>
       -Kernel 2.6
       insmod dm9ks.ko <parameter>=<value>
  ...
  using this syntax:
     insmod dm9ks.[k]o [<parameter>=<VAL1>,<VAL2>,...] 
  ...
  - mode (Media Speed)
    ...
    ex.
    	insmod dm9ks.o           ;;Auto Detection Mode
   ...
   C-2: For Redhat, You can use the following to Activate NIC
	...
	1.  login as root.
	2.  copy dm9ks.[k]o into /lib/modules/2.4.x[2.6.x]/kernel/drivers/net/
	3.  add the new line with "alias ethx dm9ks" in "/etc/module.conf". 
...
/*
 2006-07-05 port for aesop-2440 3rd edition
            godori(www.aesop-embedded.org)
            
 2008.08.08 fix for 2.6.24 mac address validate check....            
*/             
...
#ifdef CONFIG_ARCH_POLLUX // ghcstop add
	#include <linux/irq.h>
...
#if 1
#define DM9KS_MIN_IO		        (MDK_ETH_VIO_BASE) // cs8900's base addr = nGCS3 = dm9000a's base address
#define DM9KS_MAX_IO                (MDK_ETH_VIO_BASE + 0x70)
#define DM9K_IRQ		            IRQ_GPIO_C(13)
#define POLLUX_LAN_IRQ_TYPE         IRQT_HIGH 
#else
#define DM9KS_MIN_IO				(PB_ETHER_VIRT_BASE) 
#define DM9KS_MAX_IO    			(PB_ETHER_VIRT_BASE + 16)
#define DM9K_IRQ					(PB_INTC_GPIO_C_COUNT + 13 )
#endif
...
//struct net_device * dmfe_probe1(void)
{
	...
	if(!dev)
		return ERR_PTR(-ENOMEM);
     	...
     	// SET_MODULE_OWNER(dev); // defined in linux/netdevice.h of 2.6.22 kernel ==> do { } while (0)
    do { } while (0); 	
...
int __init dmfe_probe(struct net_device *dev)
{
	...
	u16 i, dm9000_found = FALSE;
		...
		set_irq_type(DM9K_IRQ, POLLUX_LAN_IRQ_TYPE);	
	#endif
	...
	do {
		writeb(DM9KS_VID_L, iobase);
		id_val = readb(iobase + 4);
		writeb(DM9KS_VID_H, iobase);
		id_val |= readb(iobase + 4) << 8;
		writeb(DM9KS_PID_L, iobase);
		id_val |= readb(iobase + 4) << 16;
		writeb(DM9KS_PID_H, iobase);
		id_val |= readb(iobase + 4) << 24;
...
// ghcstop_caution..............==> 2.6.24 커널서부터는 net/ethernet/eth.c의 eth_validate_addr()에서 mac address의 유효성을 검사한다.
// 이 것을 net/core/dev.c의 dev_open()에서 다시 검사함.......지랄맞군.....그래서 제대로 된 mac이 아니면 튕겨냄
		db->srom[0] = 0x08;
		db->srom[1] = 0x00;
		db->srom[2] = 0x3e;
		db->srom[3] = 0x26;
...
gprintk("1\n");	
	...
	if (request_irq(dev->irq,&dmfe_interrupt,SA_SHIRQ,dev->name,dev)) 
		return -EAGAIN;
#else
	if (request_irq(dev->irq, &dmfe_interrupt, IRQF_SHARED | IRQF_TRIGGER_HIGH/* IRQF_TRIGGER_RISING*/, dev->name, dev))
         return -EAGAIN;
#endif
...
static int dmfe_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	writeb(DM9KS_MWCMD, db->io_addr); // Write data into SRAM trigger
	switch(db->io_mode)
	{
		case DM9KS_BYTE_MODE:
			for (i = 0; i < skb->len; i++)
				writeb((data_ptr[i] & 0xff), db->io_data);
			break;
...
static void dmfe_packet_receive(struct net_device *dev)
{
	...
	do {
		...
		GoodPacket = TRUE;
		writeb(DM9KS_MRCMD, db->io_addr);

		/* Read packet status & length */
		switch (db->io_mode) 
			{
			  case DM9KS_BYTE_MODE: 
 				    *ptr = readb(db->io_data) + 
				               (readb(db->io_data) << 8);
				    *(ptr+1) = readb(db->io_data) + 
					    (readb(db->io_data) << 8);
				    break;
			  case DM9KS_WORD_MODE:
				    *ptr = inw(db->io_data);
